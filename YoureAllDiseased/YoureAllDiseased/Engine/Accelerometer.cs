...
namespace YoureAllDiseased
{
    ...
    public static class Accelerometer
    {
        ...
        /// </summary>
        public static Vector3 calibration = Vector3.Zero;
        ...
        private static void sensor_ReadingChanged(object sender, Microsoft.Devices.Sensors.AccelerometerReadingEventArgs e)
        {
            ...
            lock (threadLock)
            {
                //landscape left coords (adjust Y coordinate's relative acceleration to X)
                nextValue = new Vector3(-(float)e.Y * 2, -(float)e.X, (float)e.Z);
                nextValue.Normalize();
            }
        }
#endif
